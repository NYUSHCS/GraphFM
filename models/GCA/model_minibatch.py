...
from .utils import drop_feature, drop_edge_weighted, drop_feature_weighted_2, get_weights
...
class GRACE(torch.nn.Module):
    def __init__(self, num_feats, num_hidden, activation,
                              base_model, k, num_proj_hidden, tau: float = 0.5):
        super(GRACE, self).__init__()

        self.convs = [base_model(num_feats, 2 * num_hidden).jittable()]
        for _ in range(1, k - 1):
            self.convs.append(base_model(2 * num_hidden, 2 * num_hidden))
        self.convs.append(base_model(2 * num_hidden, num_hidden))
    ...
    def model_train(self, x: torch.Tensor, edge_index: torch.Tensor) -> torch.Tensor:
        self.activation = self.activation.to(x.device)
        for i in range(self.k):
            self.convs[i] = self.convs[i].to(x.device)
    ...
    def train_net(self, input_dict):
        ...
        for batch in train_loader:
            optimizer.zero_grad()
            drop_weights, feature_weights = get_weights(args, batch, self.device)
            def drop_edge(idx: int):
                if drop_scheme == 'uniform':
                    return dropout_adj(batch.edge_index.to(self.device), p=input_dict[f"drop_edge_rate_{idx}"])[0]
                elif drop_scheme in ['degree', 'evc', 'pr']:
                    return drop_edge_weighted(batch.edge_index.to(self.device), drop_weights, p=input_dict[f"drop_edge_rate_{idx}"],
                                              threshold=0.7)
                else:
            ...
            x_1 = drop_feature(batch.x.to(self.device), drop_feature_rate_1)
            x_2 = drop_feature(batch.x.to(self.device), drop_feature_rate_2)

            if drop_scheme in ['pr', 'degree', 'evc']:
                x_1 = drop_feature_weighted_2(batch.x.to(self.device), feature_weights, drop_feature_rate_1)
                x_2 = drop_feature_weighted_2(batch.x.to(self.device), feature_weights, drop_feature_rate_2)
    ...
    def inference(self, x_all, subgraph_loader):
        ...
        pbar = tqdm(total=x_all.size(0) * self.k)
        pbar.set_description('Evaluating')

        # Compute representations of nodes layer by layer, using *all*
        # available edges. This leads to faster computation in contrast to
        # immediately computing the final representations of each batch.
        self.activation = self.activation.to(x_all.device)
    ...
    def mem_speed_bench(self, input_dict):
        ...
        for batch in train_loader:
            ...
            drop_weights, feature_weights = get_weights(args, batch, device)
            def drop_edge(idx: int):
                if drop_scheme == 'uniform':
                    return dropout_adj(batch.edge_index.to(device), p=input_dict[f"drop_edge_rate_{idx}"])[0]
                elif drop_scheme in ['degree', 'evc', 'pr']:
                    return drop_edge_weighted(batch.edge_index.to(device), drop_weights,
                                              p=input_dict[f"drop_edge_rate_{idx}"],
                                              threshold=0.7)
                else:
                    raise Exception(f'undefined drop scheme: {drop_scheme}')
            ...
            x_1 = drop_feature(batch.x.to(device), drop_feature_rate_1)
            x_2 = drop_feature(batch.x.to(device), drop_feature_rate_2)

            if drop_scheme in ['pr', 'degree', 'evc']:
                x_1 = drop_feature_weighted_2(batch.x.to(device), feature_weights, drop_feature_rate_1)
                x_2 = drop_feature_weighted_2(batch.x.to(device), feature_weights, drop_feature_rate_2)

            for i in range(self.k):
                x_1 = self.activation(self.convs[i](x_1.to(device), edge_index_1))
            z1 = x_1
            for i in range(self.k):
                x_2 = self.activation(self.convs[i](x_2.to(device), edge_index_2))
            z2 = x_2
            ...
            x_all = batch.x
            for i in range(self.k):
                x_all = self.activation(self.convs[i](x_all.to(device), batch.edge_index))
            out = x_all
