...
from models.GraphMAE.utils import create_activation
from models.GraphMAE.model_minibatch import node_classification_evaluation
...
class GraphMAE2(_GraphModels):
    def __init__(self, args, data):
        ...
        self.gat_layers.append(GATConv(
            ...
            self.feat_drop, self.attn_drop, self.negative_slope, self.residual, create_activation(self.activation),
            norm=self.norm,
            concat_out=True))
        # hidden layers
        ...
        self.LPDecoder = LPDecoder(self.num_hidden, args.decode_channels_lp, 1, args.num_layers_lp, args.dropout_lp)
        ...
        self.encoder_ema = self.gat_layers

        self.encoder_ema.load_state_dict(self.gat_layers.state_dict())
    ...
    def print_num_parameters(self):
        num_encoder_params = [p.numel() for p in self.gat_layers.parameters() if p.requires_grad]
    ...
    def train_net(self, input_dict):
        ...
        for batch in train_loader:
            ...
            target_nodes = torch.arange(batch.ndata["feat"].shape[0], device=batch.ndata["feat"].device, dtype=torch.long)
            optimizer.zero_grad()
            loss = self.mask_attr_prediction(batch, batch.ndata["feat"], target_nodes, epoch, drop_g1, drop_g2)
    ...
    def mask_attr_prediction(self, g, x, targets, epoch, drop_g1=None, drop_g2=None):
        ...
        for i, conv in enumerate(self.gat_layers):
            ...
            use_x = conv(use_g, use_x)

        enc_rep = self.head(use_x)

        with torch.no_grad():
            drop_g2 = drop_g2 if drop_g2 is not None else g
            ema_x = x
            for i, conv in enumerate(self.encoder_ema):
                ema_x = conv(drop_g2, ema_x)

            latent_target = self.head(ema_x)
    ...
    def ema_update(self):
        ...
        update(self.gat_layers, self.encoder_ema)
    ...
    def mem_speed_bench(self, input_dict):
        ...
        drop_g1 = input_dict["drop_g1"]
        drop_g2 = input_dict["drop_g2"]
        model = self.gat_layers
        ema_model = self.encoder_ema
        head = self.head
        mask_rate = self._mask_rate
        projector_ema = self.projector_ema
        projector = self.projector
        predictor = self.predictor
        encoder_to_decoder = self.encoder_to_decoder
        remask_method = self._remask_method
        num_remasking = self._num_remasking
        remask_rate = self._remask_rate
        random_remask = self.random_remask
        decoder = self.decoder
        criterion = self.criterion
        fixed_remask = self.fixed_remask
        torch.cuda.empty_cache()
        ...
        for batch in train_loader:
            ...
            batch = dgl_data[0]
            batch = dgl.add_self_loop(batch).to(device)
            x = batch.ndata["feat"]
            targets = torch.arange(x.shape[0], device=x.device, dtype=torch.long)

            def mask_attr_prediction(g, x, targets, epoch, drop_g1=None, drop_g2=None):

                pre_use_g, use_x, (mask_nodes, keep_nodes) = encoding_mask_noise(g, x, mask_rate)
                use_g = drop_g1 if drop_g1 is not None else g
                ...
                with torch.no_grad():
                    drop_g2 = drop_g2 if drop_g2 is not None else g
                    ema_x = x
                    for i, conv in enumerate(ema_model):
                        ema_x = conv(drop_g2, ema_x)

                    latent_target = head(ema_x)

                    if targets is not None:
                        latent_target = projector_ema(latent_target[targets])
                    else:
                        latent_target = projector_ema(latent_target[keep_nodes])

                if targets is not None:
                    latent_pred = projector(enc_rep[targets])
                    latent_pred = predictor(latent_pred)
                    loss_latent = sce_loss(latent_pred, latent_target, 1)
                else:
                    latent_pred = projector(enc_rep[keep_nodes])
                    latent_pred = predictor(latent_pred)
                    loss_latent = sce_loss(latent_pred, latent_target, 1)

                # ---- attribute reconstruction ----
                origin_rep = encoder_to_decoder(enc_rep)

                loss_rec_all = 0
                if remask_method == "random":
                    for i in range(num_remasking):
                        rep = origin_rep.clone()
                        rep, remask_nodes, rekeep_nodes = random_remask(use_g, rep, remask_rate)
                        recon = decoder(pre_use_g, rep)

                        x_init = x[mask_nodes]
                        x_rec = recon[mask_nodes]
                        loss_rec = criterion(x_init, x_rec)
                        loss_rec_all += loss_rec
                    loss_rec = loss_rec_all
                elif remask_method == "fixed":
                    rep = fixed_remask(g, origin_rep, mask_nodes)
                    x_rec = decoder(pre_use_g, rep)[mask_nodes]
                    x_init = x[mask_nodes]
                    loss_rec = criterion(x_init, x_rec)
                else:
                    raise NotImplementedError
            ...
            def encoding_mask_noise(g, x, mask_rate=0.3):
                num_nodes = g.num_nodes()
                perm = torch.randperm(num_nodes, device=x.device)
            ...
            loss = mask_attr_prediction(batch, x, targets, 0, drop_g1, drop_g2)
