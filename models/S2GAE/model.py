...
class S2GAE(_GraphModels):
    ...
    def test(self, h, predictor, x, full_adj_t, pos_test_edge, neg_test_edge, batch_size):
        predictor.eval()

        h = [h_.to(x.device) for h_ in h]
    ...
    def train_net(self, input_dict):
        ...
        self.pos_test_edge = input_dict["test_edge"]
        self.neg_test_edge = input_dict["test_edge_neg"]
    ...
    def lp_eval_net(self, h):
        results = self.lp_test(h, self.emb_predictor.to(self.device), self.eval_batch_size)
        return results
    ...
    def embed(self, data):
        import copy
        tmp_encoder = copy.deepcopy(self.emb_model).eval().cpu()
        return tmp_encoder(data.x.cpu(), data.full_adj_t.cpu())
    ...
    def mem_speed_bench(self, input_dict):
        ...
        if self.mask_type == 'um':
            adj, _, pos_train_edge = edgemask_um(self.mask_ratio, edge_index, device, data.x.shape[0])
        else:
            adj, _, pos_train_edge = edgemask_dm(self.mask_ratio, edge_index, device, data.x.shape[0])
        adj = adj.to(device)
        out = self.model(data.x, adj)
